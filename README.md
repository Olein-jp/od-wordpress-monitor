...
- OD WordPress Monitor：1.0.11
- OD Monitor Agent：1.0.4
- 通信スキーマ：1.0

## バージョン別アップデート状況

各プラグインは独立して更新できます。MonitorとAgentのバージョンが異なっていても、通信スキーマ1.0の互換性が保たれている範囲では利用できます。

### OD WordPress Monitor

| バージョン | 主な変更点 |
| --- | --- |
| 1.0.11 | 通知先の保存後もテスト通知ボタンが無効のままになる問題を修正。 |
...
| 1.0.4 | Dashboardの状態集計とScheduler健全性、Sitesの状態フィルター、サイト詳細画面のイベント・監視結果表示を追加。 |
...
初回導入または手動更新では、各プラグインの最新リリースからZIPをダウンロードしてWordPress管理画面の「プラグイン」→「新規プラグインを追加」→「プラグインのアップロード」からインストールしてください。既存サイトの更新では、管理画面の更新通知を利用するか、同じ手順で新しいZIPをアップロードします。MonitorとAgentは別々に更新されるため、片方だけに更新通知が表示される場合があります。
