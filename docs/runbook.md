...
Monitorのアンインストールは保存データを削除しません。削除前のバックアップ対象、auth saltを含む秘密情報の取扱い、復元順序、マイグレーションと接続確認は[Monitorのアンインストール・バックアップ・復元](backup-and-restore.md)に従ってください。復元が完了するまでは、バックアップや既存credentialを上書きせず、管理画面のデータベース更新状態と各サイトの「Test Connection」を確認します。

## データベース更新の失敗

Monitorの更新後は、次の通常リクエストで必要なデータベース変更を自動適用します。処理中は同時実行を期限付きlockで防ぎ、完了するまで監視処理を開始しません。現在のスキーマバージョンであれば変更処理は実行されません。

管理画面にデータベース更新失敗の通知が表示された場合は、WordPressとデータベースのエラーログ、データベースユーザーのテーブル作成・変更権限、空き容量を確認してください。原因を解消して管理画面を再読み込みすると、旧バージョンから同じマイグレーションを再試行します。失敗時にスキーマバージョンは更新されず、登録サイト、credential、監視履歴を削除またはロールバックしません。復旧前にoption値やテーブルを手動変更せず、通常のバックアップ運用に従って必要な退避を行ってください。
