...
- `odm_notification_channel_settings`（Slack・Discordの暗号化済みWebhook URL、Chatworkのroom IDと暗号化済みAPIトークン。autoloadなし）
