...
Issue #42ではこの方針を次のように実装する。SSLの`healthy -> warning`だけは既存の状態変化通知として即時送信し、`unknown -> warning`、継続中の`warning -> warning`、`critical -> warning`は送信しない。更新情報とSite Healthのrecommendedは、管理画面で個別に有効化した場合だけ、WordPressサイトのタイムゾーンにおける指定時刻（既定09:00）の日次まとめへ含める。

日次まとめは、前回まとめ以降に初めて現れた、または内容が変わった警告だけを含める。同じ警告が継続している場合は毎日繰り返さず、解消した項目は次回の署名状態から削除する。更新情報の署名はWordPressコアの更新数と、ソフトウェアの`id/current_version/latest_version`だけを順序正規化して作成し、`collected_at`や表示名を除外する。Site Healthはrecommendedテストの`id/status`だけを同様に正規化する。署名と日付のclaimは非autoload optionへ保存し、cronが同じ時刻に重複起動しても1日1回に抑える。1回のまとめで走査するサイト数は100件に制限し、通知本文にレスポンス全文や秘密情報は含めない。
...
Issue #43の実装では、通知配送専用の`odm_retry_notification_delivery` hookを使用する。408、429、5xx、timeout、接続失敗だけを1回再送し、`Retry-After`が1〜900秒なら採用する。それ以外は60秒後に再送する。400、401、403、404、設定・payloadの不正は対象外とする。cron引数はイベントIDとチャネルIDだけで、通知本文や宛先を保存しない。再送前にイベントの配送記録と現在のチャネル設定を読み直し、成功済み・再送済み・無効化済みなら送信しない。重複実行を防ぐ非autoloadの期限付きclaimは既存のlock cleanupで削除する。再送結果はイベントの該当チャネルだけへ結合し、監視イベントは追加しない。日次まとめはイベントIDを持たないため、この再送の対象外とする。
