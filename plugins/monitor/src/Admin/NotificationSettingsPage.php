...
final class NotificationSettingsPage {
	...
	public function handle_test(): void {
		...
		if ( ! in_array( $channel_id, array( SlackNotifier::CHANNEL_ID, DiscordNotifier::CHANNEL_ID ), true ) ) {
	...
	private function render_test_forms(): void {
		...
		foreach ( array( SlackNotifier::CHANNEL_ID, DiscordNotifier::CHANNEL_ID ) as $channel_id ) {
			?>
			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
				...
				<?php submit_button( sprintf( /* translators: %s: service name. */ __( 'Send %s test notification', 'od-wordpress-monitor' ), ucfirst( $channel_id ) ), 'secondary', 'submit', false, array( 'disabled' => ! $this->channel_settings->has_webhook( $channel_id ) ) ); ?>
			</form>
			<?php
		}
	}
	...
	private function render_test_notice(): void {
		...
		$message = $success
			...
			: __( 'The test notification could not be sent. Check the saved webhook and try again.', 'od-wordpress-monitor' );
